...
;; Diffrences between the old to new domain:
;; 1. In action build-ship - Added requirement for wharf - better logic
;; 2. In action mine-ore- Added 1 to labour, I think it's needed logically
...
(define (domain civ)
  ...
  (:action build-ship
   ...
   :precondition (and (potential ?v) 
                      (has-wharf ?p)  ; Added requirement for wharf - better logic
                      (>= (available iron ?p) 4))
   :effect (and (decrease (available iron ?p) 4)
                (is-at ?v ?p)
                (is-ship ?v)
                (not (potential ?v))
  ...
  (:action mine-ore
   ...
   :effect (and (increase (available ore ?p) 1)
                (increase (labour) 1)       ;; added labour, I think it's needed logically
